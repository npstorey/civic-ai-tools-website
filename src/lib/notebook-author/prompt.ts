 ...
 * config (ADR-0020, `src/lib/site-config.ts`) — demo defaults keep the
 * emitted bytes identical when no config is set.
 */
...
export function buildCell0Source(args: {
  ...
  return [
    ...
    `**Query:** ${args.query}  `,
    portalLine.trimEnd(),
    `**Generated:** ${args.generatedAt} via [${getPublicationHost()}](${getEvidenceSiteOrigin()})`,
    '',
...
export function buildFooterCellSource(args: {
  ...
  lines.push(
    ...
    '',
    `Generated by [${getPlatformTitle()}](${getEvidenceSiteOrigin()}).`,
  );
  return lines.join('\n');
}
