/**
 * Guard: no file this repository tracks spells an open-data portal hostname
 * unless it is classified as something other than a default (#407).
 *
 ...
 *   - Only the extensions in `SCANNED_EXTENSIONS` are read. JSON, CSV, YAML and
 *     `.env*` templates are tracked too and are NOT in this universe.
 *
 * THE UNIVERSE IS `git ls-files`, NOT A DIRECTORY (Wave N11 #434 F-W). This
 * guard used to walk `src/` with `readdirSync`, so a default written anywhere
 * else was invisible to it: a default portal literal placed in `scripts/` went
 * green on CI (draft #462, part 1), and five tracked files outside `src/`
 * already spelled a hostname nobody had classified. It now reads every tracked
 * file of the scanned extensions, wherever it sits, tests excluded — the
 * `model-call-registry.test.ts` precedent (#363) — so a directory that does
 * not exist yet is in scope the day it lands. Paths below are repository
 * paths, and one path may carry more than one classification; the hit counts
 * of a path's entries sum.
 *
...
interface Exception {
  /** Repository path, as `git ls-files` prints it. */
...
/**
 * The classified universe. Under `src/`, measured at `77e51bd`: 102 hits across 24 files, of
 ...
 * listed by path.
 *
 * Outside `src/`, measured at `b42acdf` when the universe became `git ls-files`:
 * 32 hits across 5 files, classified at the foot of the list. One of them,
 * `scripts/rehearse-instance-identity.ts`, handed `generateNotebook` a hostname
 * in its run-portal slot; that argument is now `null` rather than classified,
 * because a hostname in a run-input slot is the one thing no classification
 * may cover, even where the call it rehearses names the same portal.
 */
const EXCEPTIONS: readonly Exception[] = [
  {
    path: 'src/lib/bpmn/traces.ts',
  ...
  {
    path: 'src/lib/mcp/socrata-skill.ts',
  ...
  {
    path: 'src/components/QueryForm.tsx',
    classification: 'reference-table',
    hits: 5,
  ...
  {
    path: 'src/lib/notebook-author/tool-to-cell.ts',
  ...
  {
    path: 'src/components/SkillPromptDisclosure.tsx',
    classification: 'reference-table',
    hits: 3,
  ...
  {
    path: 'src/lib/mcp/boston-skill.ts',
    classification: 'server-endpoint',
    hits: 3,
  ...
  {
    path: 'src/lib/notebook-author/helpers/fetch_opencontext.py',
    classification: 'server-endpoint',
    hits: 3,
  ...
  {
    path: 'src/components/notebook/__dev__/sampleExecutedNotebook.ts',
    classification: 'fixture',
    hits: 2,
  ...
  {
    path: 'src/app/(app)/dev/notebook-preview/NotebookPreviewClient.tsx',
    classification: 'fixture',
    hits: 1,
  ...
  {
    path: 'src/components/notebook/NotebookOutput.tsx',
  ...
  {
    path: 'src/lib/notebook-author/fixtures/README.md',
    classification: 'documentation',
    hits: 1,
  ...
  {
    path: 'scripts/eval-models.mjs',
    classification: 'fixture',
    hits: 15,
    why: "The evaluation's question set: each question names its city in its own text and carries that city's portal beside it, so a harness run for that question starts on the portal the question is about; it is the harness's input data, not an instance's default, and nothing in the app runs from it.",
  },
  {
    path: 'scripts/eval-models.mjs',
    classification: 'reference-table',
    hits: 7,
    why: "Worked examples of the `portal` argument in the harness's own copy of the tool description and its `portal` parameter description — shapes offered to the model, as `src/lib/mcp/tools.ts` offers them.",
  },
  {
    path: 'scripts/rehearse-instance-identity.ts',
    classification: 'fixture',
    hits: 2,
    why: "The portal a synthetic rehearsal call names in its own arguments, on a `.example.gov` host, once for the notebook and once for the package; the rehearsal's run-portal argument is `null`, so no run inherits it.",
  },
  {
    path: 'docs/project-plan.md',
    classification: 'documentation',
    hits: 4,
    why: 'Early planning prose: a sketched request body and trace, and an open question about whether queries should default to a portal — the question this guard now answers.',
  },
  {
    path: 'docs/RETROSPECTIVE.md',
    classification: 'documentation',
    hits: 2,
    why: 'Session retrospectives naming the Data Commons endpoint and the Boston server domain while recounting how #407 and its probes were measured.',
  },
  {
    path: 'docs/api/records-publish.md',
    classification: 'documentation',
    hits: 1,
    why: 'A worked publish request in the API contract, whose tool call names the portal it addressed.',
  },
];
...
const OUT_OF_SCOPE = /\.test\.(?:ts|tsx|js|jsx|mjs)$/;
...
/** Every tracked file of the scanned extensions, tests excluded, as repository paths. */
function sourceFiles(): string[] {
  const tracked = execFileSync('git', ['ls-files', '-z', '--full-name'], {
    ...
    .split('\0')
    .filter(Boolean);
  // A path the index tracks but the working tree has deleted is not a file to read.
  return tracked.filter(
    (name) => SCANNED_EXTENSIONS.test(name) && !OUT_OF_SCOPE.test(name) && fs.existsSync(path.join(REPO_ROOT, name)),
  );
}
...
function portalHits(): Hit[] {
  ...
  assert.ok(
    files.some((f) => !f.startsWith('src/')) && files.length > 300,
    `PREMISE: git ls-files yielded ${files.length} scanned files; the universe is not the tree`,
  );
...
test('no tracked file spells a portal hostname unless the exception list classifies it', () => {
...
test('every exception still describes the tree, hostname for hostname', () => {
  ...
  for (const entry of EXCEPTIONS) claimed.set(entry.path, (claimed.get(entry.path) ?? 0) + entry.hits);
  const drifted = [...claimed].flatMap(([entryPath, hits]) => {
    const actual = counted.get(entryPath) ?? 0;
    const classes = EXCEPTIONS.filter((e) => e.path === entryPath).map((e) => `${e.hits} ${e.classification}`).join(' + ');
    return actual === hits ? [] : [`${entryPath} — classified ${classes}, found ${actual}`];
  });
