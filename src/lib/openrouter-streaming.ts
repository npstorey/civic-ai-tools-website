...
function failureFacts(error: unknown): string {
  // The bound itself lives in `streaming.ts` (#503 WF): one implementation for
  // every server log line that reports a failure. This only formats it.
  const { errorClass, status } = errorLogFacts(error);
  const statusNote = status !== undefined ? ` [status ${status}]` : '';
  return `${statusNote} [error class: ${errorClass}]`;
}
 ...
 * the classified kind, and the server log above gets the kind and the
 * bounded `failureFacts` — never the raw error (#503).
 *
