...
> **Historical — audit run 2026-01-15, not repeated.** (The date line below says 2025-01-15; the year is a typo — this repo's first commit is 2026-01-14, and this file entered git on 2026-03-05.) Model availability, pricing, and tool-calling reliability have turned over completely since. Retained for the evaluation criteria, not the verdicts. The audit below is about OpenRouter's roster because that is the endpoint this project ran on at the time, and it still is by default — but it is no longer the only one an instance can use, so read the roster as one endpoint's, not as the app's.
>
> The live model list is no longer defined in code: since [#30](https://github.com/npstorey/civic-ai-tools-website/issues/30) it is an instance's **model catalog** (`MODEL_CATALOG` / `MODEL_CATALOG_PATH`, served via `/api/models`), with a built-in list kept only while the instance runs against the default endpoint. The endpoint, its wire dialect and the key are environment-driven (`MODEL_API_BASE_URL`, `MODEL_API_KIND`, `MODEL_API_KEY`) — see [`deploy.md`](deploy.md#the-model-seam) and [`instance-setup.md` §5](instance-setup.md#5-point-the-instance-at-a-model-endpoint).
