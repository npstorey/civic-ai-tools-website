...
They are three kinds, not one: a check that could only pass (1–5, 7–9), a probe that could only return absence (6), and a check whose scope excluded the defect it names, so that passing said nothing about it (10, 11). The third is the dangerous one, because it does not merely fail to find a defect — it reports one fixed: 10's probe licensed the claim that `replayPortalForPackage` was repaired, and 11's guard passes on the exact fallback the phase that amended its file had just removed.
