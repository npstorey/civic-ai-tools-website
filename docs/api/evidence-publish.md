...
| Publication-record creation (`POST /api/evidence/:slug/publish`) | **Live** (Phase 2, 2026-06-12) — `runEvaluation` accepted but inert until Phase 3 |
...
- **`POST /api/evidence/:slug/publish`** — authorization: publisher-only (the record's creator, via bearer token with `evidence:publish` scope or session cookie; delegated-publisher is a future ADR per Q20). Body: `{ "runEvaluation"?: boolean }` — accepted for forward-compatibility but **inert until Phase 3** wires the adversarial-eval gate; no evaluation runs today and no `evaluationNodeId` is returned. Flow: the content is re-homed from its random committed key to the canonical hash-addressed key, then two independently signed + timestamped + Rekor-included nodes are emitted:
  ...
  Returns `{ published: true, publishesNodeId, locatedAtNodeId, url }`. On pair-emission failure the canonical blob is rolled back and the record stays committed (`500`; retryable — never half-published).
