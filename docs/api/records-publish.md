...
| `failed`         | boolean  | no       | `true` when the producer recorded the call as rejected — the source either did not answer it, or answered it with a failure (a JSON-RPC `error`, or a result carrying `isError: true`, the MCP specification's tool-level failure). Send an `isError` result as `failed: true` with `failureKind: "unknown"`, never as an answered call: the flag carries no cause, so none is asserted. Carried into `queries[].failed`. Omit it (never send `false`) for a call not recorded as failed: absent means "not recorded as failed", and a package with no rejected call is byte-identical to one built before this field existed. |
...
- **`datHere` requirements (OES §9.1.1)** — When `contentProfile === "datHere"`, the route additionally enforces:
    ...
    3. The packager auto-emits `extensions["org.civicaitools.environment"]` with section-C metadata: `modelVersion` (from `model`), `temperature` (prototype placeholder `0` pending capture work), `mcpServers` (for a `chat-flow-stream` capture — a run this instance made — every MCP server the instance routes to, in its routing order, each as `{ "url": <the address as configured>, "name": <source id> }`: `socrata` from `SOCRATA_MCP_URL` when that is set, then `data-commons` and `boston-opencontext` from their variables or coded defaults, so two or three entries; for any other `captureMethod`, the single `{ "url": skillMetadata.mcpServerUrl }` as before, or `[]` when the trace records none. The same rule sets each data source's `civic:serverUrl` in the provenance graph: the configured address for a `chat-flow-stream` capture, and otherwise no address except the skill source's, taken from the trace. See [`docs/deploy.md`](../deploy.md) for what each variable makes a record say), `toolDefinitions` (prototype placeholder `[]`), `host` (derived from `PUBLISHER_PUBLICATION_HOST` — see [`docs/instance-setup.md`](../instance-setup.md); `"civicaitools.org"` on the reference deployment).
...
- **2026-09-11** — **A source that answers with a refusal is a rejected call** (civic-ai-tools-website#429, ruled in civic-ai-tools-website#434). The `failed` row of the tool-call table no longer reads only "the source did not answer it": a `tools/call` result carrying `isError: true` — the MCP specification's tool-level failure — is an answer that refuses, and this publisher now records one as a rejected call. **An external producer sends such a result as `failed: true` with `failureKind: "unknown"`, never as an answered call with a `resultSummary`** — MCP's flag carries no cause, so none is asserted. No schema change and no wire break: `failed` and `failureKind` are unchanged in shape and meaning, and a package with no rejected call is byte-identical to one built before this revision.
