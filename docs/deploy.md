...
| `SOCRATA_MCP_URL` | Every data query refuses with a typed error naming this variable. No fallback — this used to default to the project's hosted endpoint, which silently routed an unconfigured instance's queries through infrastructure it does not operate. Set it to the Socrata MCP deployment this instance should query. Pointing it at another operator's endpoint (the project's hosted one included) is a real choice, but understand what it means: your users' queries route through that host's infrastructure, not yours. **What it makes a signed record say:** for a run this instance makes (`captureMethod: "chat-flow-stream"`), the value exactly as set (not normalised: a bare host stays a bare host, with no `/mcp` appended) is the Socrata source agent's `civic:serverUrl` in the provenance graph, and under the datHere profile the `socrata` entry of `extensions["org.civicaitools.environment"].mcpServers`. All three MCP addresses are recorded as configured, so a deployment that points one at a container-internal name (a sibling service reached as `http://socrata-mcp:8000`, say) publishes that name in its signed records. A record of a run made elsewhere (a `claude-code-*` capture) takes no address from this instance's configuration. The addresses are read when the record is packaged, so a record published after a configuration change names the new values. |
...
| `DATA_COMMONS_MCP_URL` + `DATA_COMMONS_API_KEY` | The Data Commons data source — its tool calls fail without the key (the hosted endpoint mandates it). `DC_API_KEY` is the separate key passed into *executed notebooks* for their own Data Commons requests. **What the address makes a signed record say:** for a run this instance makes, the value as set, or when unset the coded default (Google's public endpoint, in `src/lib/mcp/registry.ts`), is the `data-commons` entry of the datHere `mcpServers` list on every record, and the Data Commons agent's `civic:serverUrl` on a record whose run called a Data Commons tool. The record's `dataSources` entry for Data Commons keeps the packaging library's fixed portal value, that same public endpoint, whatever this is set to. The key is never recorded. |
| `BOSTON_OPENCONTEXT_MCP_URL` | The Boston OpenContext data source. It has a coded default, `https://data-mcp.boston.gov/mcp`, so unset means that public endpoint, not no source. **What it makes a signed record say:** for a run this instance makes, the value as set, or that default, is the `boston-opencontext` entry of the datHere `mcpServers` list on every record, and the Boston OpenContext agent's `civic:serverUrl` on a record whose run called one of its tools. |
