...
| `EVIDENCE_SIGNER_BINDING_TIER` | **Required to sign.** Envelope `signer.bindingTier` (§8.5) — `platform` for a standard instance. | none — refuse to sign |
| `EVIDENCE_SIGNER_IDENTIFIER` | **Required to sign.** Envelope `signer.identifier` — must match the registry entry (check #14). | none — refuse to sign |
| `EVIDENCE_SIGNER_DISPLAY_NAME` | **Required to sign.** Envelope `signer.displayName` — must match the registry entry. | none — refuse to sign |
| `EVIDENCE_PLATFORM_AGENT_TITLE` | **Required to sign.** PROV platform-agent title and the "Generated by …" attribution name in notebooks. | none — refuse to sign |
...
| `EVIDENCE_PLATFORM_AGENT_ID` | PROV platform-agent id inside the signed provenance graph. | the publication host |
| `EVIDENCE_PLATFORM_AGENT_URL` | PROV platform-agent URL. | `EVIDENCE_SITE_ORIGIN` |
