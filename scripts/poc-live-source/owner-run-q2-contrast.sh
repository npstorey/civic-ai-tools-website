...
#
#        op run --env-file=<env file> -- sh scripts/poc-live-source/owner-run-q2-contrast.sh
#
...
#     sh scripts/poc-live-source/owner-run-q2-contrast.sh
#
#   sh scripts/poc-live-source/owner-run-q2-contrast.sh [path-to-env-file]   # literal fallback
set -eu
...
if [ -z "$KEY" ]; then
  if [ ! -f "$ENV_FILE" ]; then
    ...
    echo "  op run --env-file=<env file> -- sh scripts/poc-live-source/owner-run-q2-contrast.sh"
    exit 1
  fi
  ...
  case "$KEY" in
    op://*)
      ...
      echo "  op run --env-file=\"$ENV_FILE\" -- sh scripts/poc-live-source/owner-run-q2-contrast.sh"
      echo
      echo "nothing was created and nothing was billed."
      exit 1
      ;;
...
if [ -n "${POC_STEP0_ONLY:-}" ]; then
  exec env -i PATH="$PATH" HOME="$HOME" MODEL_API_KEY="$KEY" POC_STEP0_ONLY=1 \
    node --no-warnings scripts/poc-live-source/run-q2-prompt-contrast.mjs
fi
